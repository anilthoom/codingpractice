...
public class SearchExcel {
    public static void main(String[] args) {
        try {
            ...
            ArrayList<Row> rows = searchSheet("Primitive", sheet);
    ...
    public static ArrayList<Row> searchSheet(String searchText, XSSFSheet sheet) {
        ...
        int counter = 0;
        //Iterate rows
        for (int j = sheet.getFirstRowNum(); j <= sheet.getLastRowNum(); j++) { counter++;

            XSSFRow row = sheet.getRow(j);
           
            //Iterate columns
            for (int k = row.getFirstCellNum(); k < row.getLastCellNum(); k++) { counter++;
